...
using parse_function =
    std::function<void(byte_buf_reader& reader, byte_buf_reader& hex_reader, std::shared_ptr<model::content>& content)>;
 ...
 */
template <typename T>
void handle_v(std::shared_ptr<sl651_2014::model::content>& content, byte_buf_reader& hex_reader, uint32_t data_len,
              model::data_type data_type, const std::optional<T>& opt) {
    nlohmann::json j;
