...
namespace sl651_2014 {
    ...
    namespace codec {
        ...
        // crc 计算公式
        using custom_crc_type = boost::crc_optimal<16, CRC_POLYNOMIAL, CRC_INITIAL_REMAINDER,
                CRC_FINAL_XOR_VALUE, CRC_REFLECT_INPUT, CRC_REFLECT_OUTPUT>;
        ...
        public:
            ...
            bool crc_check(const asio::const_buffer &header,
                           const asio::const_buffer &content,
                           const asio::const_buffer &end) override;

            crc_value_t crc_calculate(const asio::const_buffer &header,
                                      const asio::const_buffer &content,
                                      const asio::const_buffer &end) override;
            ...
            model::c_shared_ptr parse_content(const model::h_shared_ptr &h_ptr,
                                              const asio::const_buffer &cb);

            model::e_shared_ptr parse_end(const model::h_shared_ptr &h_ptr,
                                          const model::c_shared_ptr &c_ptr,
                                          const asio::const_buffer &eb);

            void do_something(const model::h_shared_ptr &h_ptr,
                              const model::c_shared_ptr &c_ptr,
    ...
    namespace encoder {
         ...
         * 解码器函数
         */
        using encoder_fun = std::function<void(const model::h_shared_ptr &h,
