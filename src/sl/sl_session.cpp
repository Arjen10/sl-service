...
read_some_op<AsyncReadStream, DynamicBuffer>::read_some_op(AsyncReadStream &s,
                                                           DynamicBuffer &b,
                                                           sl_req_decoder &p)
...
template<class AsyncReadStream, class DynamicBuffer>
template<class Self>
void read_some_op<AsyncReadStream, DynamicBuffer>::operator()(Self &self,
                                                              beast::error_code ec,
                        ...
                        BOOST_ASIO_CORO_YIELD
                                                            {
                                                                ...
                                                                auto const mb =
                                                                        beast::detail::dynamic_buffer_prepare(b_, size,
                                                                                                              ec,
                                                                                                              http::error::buffer_overflow);
                                                                if (ec)
                                                                    goto upcall;
                    ...
                    if (!cont_) {
                        BOOST_ASIO_CORO_YIELD {
                                                                BOOST_ASIO_HANDLER_LOCATION(
                                                                        (__FILE__, __LINE__, "sl::async_read_some"));
...
BOOST_BEAST_ASYNC_RESULT2(ReadHandler) async_read_some(AsyncReadStream &stream,
                                                       DynamicBuffer &buffer,
                                                       sl_req_decoder &parser,
                                                       ReadHandler &&handler) {
    return net::async_compose<ReadHandler,
            void(beast::error_code, std::size_t)>(
            read_some_op<AsyncReadStream, DynamicBuffer>{
                    stream,
                    buffer,
                    parser
            },
...
template<class Stream, class DynamicBuffer, class Condition>
template<class Self>
void read_op<Stream, DynamicBuffer, Condition>::operator()(Self &self,
    ...
    BOOST_ASIO_CORO_REENTER(*this) {
                    ...
                    if (Condition{}(p_)) {
                        BOOST_ASIO_CORO_YIELD {
                                                                BOOST_ASIO_HANDLER_LOCATION(
                                                                        (__FILE__, __LINE__, "http::async_read"));
...
async_read(AsyncReadStream &stream, DynamicBuffer &buffer, sl_req_decoder &decoder, ReadHandler &&handler) {
    return net::async_compose<
            ...
            read_op<AsyncReadStream, DynamicBuffer, parser_is_done>(stream, buffer, decoder),
            handler, stream);
}

sl_session::sl_session(tcp::socket &&socket, net::io_context &ioc)
        : socket_(std::move(socket)), _timer(ioc) {
...
void sl_session::run() {
    ...
    net::dispatch(socket_.get_executor(),
                  beast::bind_front_handler(&sl_session::do_read, this->shared_from_this()));
}
...
void sl_session::do_read() {
    async_read(
            ...
            buffer_,
            *codec_,
            beast::bind_front_handler(&sl_session::on_read, shared_from_this()));
}
...
void sl_session::do_write(std::optional<std::shared_ptr<asio::streambuf>> &&buffer)
{
    ...
    net::async_write(
            socket_,
            *buf_ptr,
            [self, buf_ptr](boost::system::error_code ec, std::size_t bytes_transferred) {
                if (ec) {
