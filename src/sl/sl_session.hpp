...
template <class AsyncReadStream, class DynamicBuffer> class read_some_op : net::coroutine {
    AsyncReadStream& s_;
    DynamicBuffer& b_;
    sl_req_decoder& p_;
  ...
  public:
    ...
    template <class Self> void operator()(Self& self, beast::error_code ec = {}, std::size_t bytes_transferred = 0);
};

template <class AsyncReadStream, class DynamicBuffer, BOOST_BEAST_ASYNC_TPARAM2 ReadHandler>
BOOST_BEAST_ASYNC_RESULT2(ReadHandler)
async_read_some(AsyncReadStream& stream, DynamicBuffer& buffer, sl_req_decoder& parser, ReadHandler&& handler);

template <class Stream, class DynamicBuffer, class Condition> class read_op : net::coroutine {
    Stream& s_;
    DynamicBuffer& b_;
    sl_req_decoder& p_;
    std::size_t bytes_transferred_;
