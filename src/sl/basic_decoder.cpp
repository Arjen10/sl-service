...
void union_json(nlohmann::json& ret, const nlohmann::json& h_j, const nlohmann::json& c_j, const nlohmann::json& e_j) {
    ret["header"] = h_j;
    ret["content"] = c_j;
    ret["end"] = e_j;
}
...
const std::unordered_map<method, func> _map = {
    ...
    {method::http_callback,
     [](const std::string& stcd, const nlohmann::json& h_j, const nlohmann::json& c_j, const nlohmann::json& e_j) {
         ...
         try {
             auto resp =
                 cpr::Post(cpr::Url{http_conf.value().get_url()},
...
raw_hex_value::raw_hex_value(byte_buf_reader& reader, uint16_t length, std::string note, std::string v, bool extended)
    : _note(std::move(note)), _length(length), _v(std::move(v)), _extended(extended) {
    reader.read_hex_str(this->_raw_hex, length);
}

raw_hex_value::raw_hex_value(byte_buf_reader& reader, uint16_t length, std::string note, const nlohmann::json& v,
                             bool extended)
    : _note(std::move(note)), _length(length), _v(v.get<std::string>()), _extended(extended) {
    reader.read_hex_str(this->_raw_hex, length);
}

raw_hex_value::raw_hex_value(byte_buf_reader& reader, uint16_t length, std::string note, bool extended)
