...
void mqtt5_client::init(boost::asio::io_context& ioc) {
    _client =
        std::make_shared<mqtt_client_type>(ioc, std::monostate{}, boost::mqtt5::logger(boost::mqtt5::log_level::info));
    ...
    _client->async_run([this](boost::mqtt5::error_code ec) {
        if (!ec) {
            ...
            SPDLOG_INFO("MQTT client stopped gracefully.");
            return;
        }
        SPDLOG_ERROR("MQTT connection lost: {}", ec.message());
    });
}
...
void mqtt5_client::publish_async(const std::string& stcd, const std::string& message) {
    ...
    auto publish_callback = [this, topic, message](auto ec, auto&&...) {
        if (!ec) {
            SPDLOG_DEBUG("[MQTT publish OK] topic= {}", topic);
            return;
        }
        SPDLOG_ERROR("[MQTT 发布失败] topic= {} message= {}  ec= {}", topic, message, ec.message());
    };
    ...
    case boost::mqtt5::qos_e::at_most_once:
        this->_client->async_publish<boost::mqtt5::qos_e::at_most_once>(
            topic, message, boost::mqtt5::retain_e::yes, boost::mqtt5::publish_props{},
    ...
    case boost::mqtt5::qos_e::at_least_once:
        this->_client->async_publish<boost::mqtt5::qos_e::at_least_once>(
            topic, message, boost::mqtt5::retain_e::yes, boost::mqtt5::publish_props{}, std::move(publish_callback));
        break;
    ...
    default:
        // 按道理来说不会走到这里，打个日志提醒下
        SPDLOG_WARN("错误的 qos 配置 {} ，将不会给MQTT发送任何消息", static_cast<std::int32_t>(qos));
        break;
    }
}